...
#define PUSH_DEVICE_ID XPAR_DEVICE_ID
...
/*****************************************************************************/
    ...
    XGpio_Initialize(&rgbLedInst, RGB_LED_DEVICE_ID);
    XGpio_SetDataDirection(&rgbLedInst, RGB_CHANNEL, 0x00);

    XGpio_Initialize(&pushInst, PUSH_DEVICE_ID);
    XGpio_SetDataDirection(&pushInst, PUSH_CHANNEL, 0xFF);
	xil_printf("Initialization Complete, System Ready!\n");

    xTaskCreate(vKeypadTask, "keypad", configMINIMAL_STACK_SIZE, 	/* The stack allocated to the task. */
...
static void vRgbTask(void *pvParameters)
{
	...
	const TickType_t xButtonDelay = pdMS_TO_TICKS(150);
	u32 pushVal;

	(void)pvParameters;

#if USE_PWM
	/* PWM: fixed period T (set to min flicker-free period from Task 9). */
	const TickType_t xPwmPeriod = pdMS_TO_TICKS(20);
	TickType_t xOnDelay  = xPwmPeriod / 2;
	TickType_t xOffDelay = xPwmPeriod - xOnDelay;

	xil_printf("RGB task (PWM). Buttons: 8=brighter, 1=dimmer.\r\n");

	for (;;) {
		pushVal = XGpio_DiscreteRead(&pushInst, PUSH_CHANNEL);

		if (pushVal == 8) {
			if (xOnDelay < xPwmPeriod - 1) {
				xOnDelay++;
				xOffDelay = xPwmPeriod - xOnDelay;
				xil_printf("xOnDelay: %u, xOffDelay: %u\r\n", (unsigned)xOnDelay, (unsigned)xOffDelay);
				vTaskDelay(xButtonDelay);
			}
		} else if (pushVal == 1) {
			if (xOnDelay > 1) {
				xOnDelay--;
...
#else
	/* Period control (Tasks 1-9): buttons 8=inc period, 1=dec period. */
	TickType_t xPeriod = pdMS_TO_TICKS(100);
	TickType_t xDelay  = xPeriod / 2;

	xil_printf("RGB task (period control). Buttons: 8=inc period, 1=dec period.\r\n");
